...
class IreneSTTConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Irene STT."""
    VERSION = 1
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            try:
                ...
                return self.async_create_entry(title=f"Irene STT ({unique_id})", data=user_input)
